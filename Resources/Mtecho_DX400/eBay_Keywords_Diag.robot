...
BMC_MAC_Calculate_py
    ${output}   MacAddress.mac_converter    address=${MAC}    size=${383}
    ${output}    Remove String    ${output}    -
    Set Global Variable     ${BMC_Mac}    ${output}
...
Swap_BMC
    ${status}   ${output}=  Run Keyword And Ignore Error    SSHLibrary.Read Until    Swap_BMC_1
...
Swap_COME
    ${status}   ${output}=  Run Keyword And Ignore Error    SSHLibrary.Read Until    Swap_COME_1
