...
"""This is the Serial Wti library for calling with Robot Framework.

Calling class and function in the Serial Wti library with Robot Framework.
"""
...
class Wti_ReSerial(Serial):
    """This class for interface to the UUt with serial port"""

    def __init__(self, serialport, baudrate):
        super(Wti_ReSerial, self).__init__(serialport, baudrate,
    ...
    def open(self, max_retry=5):
        ...
        for retry in range(0, max_retry):
            try:
                response = super(Wti_ReSerial, self).open()
                if response:
                    return response
    ...
    def close(self):
        super(Wti_ReSerial, self).close()

    def write(self, command, max_retry=5, no_lf=False):
        for retry in range(0, max_retry):
            try:
                super(Wti_ReSerial, self).write(command, no_lf)
                return  True
            except Exception as ex:
                if retry == (max_retry - 1):
    ...
    def send_expect_cmd(self, command, expect, time_out=20, max_retry=1):
        ...
        for retry in range(0, max_retry):
            try:
                found, response = super(Wti_ReSerial, self).send_expect_cmd(
